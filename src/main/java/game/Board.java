...
public class Board {
    ...
    List<Square> validMoves(Piece piece) {
        ...
        for (int[] dir : directions) {
            ...
            if (piece.rank == Piece.SCOUT) {
                ...
                if (isInBounds(newX, newY) && isPassable(newX, newY) && 
                		(!isOccupied(piece.side, newX, newY) || isOccupiedByOpposite(piece.side, newX, newY))) {
                    validSquares.add(this.grid[newX][newY]);
                }
            }
        }
    ...
    boolean isOccupiedByOpposite(Side side, int x, int y) {
    	return getPiece(x, y)
	        .map(piece -> piece.side == Side.getOpposite(side))
	        .orElse(false);
    }
